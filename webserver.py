...
def create():
    ...
    if request.method == WebMethod.POST:
        ...
        people = appmod.People.query.order_by(appmod.People.last_name, appmod.People.first_name).all()

        for person in people:
            if person.first_name == new_first_name and person.last_name == new_last_name:
                exists = True
                return render_template("create.html", exists=exists)

        try:
            new_person = appmod.People(first_name = new_first_name, last_name = new_last_name, dob = new_dob)
    ...
    if request.method == WebMethod.POST:
        ...
        for person in people:
            if person.first_name == prev_first_name and person.last_name == prev_last_name:
                try:
                    updated_person = appmod.People(first_name = updated_first_name, last_name = updated_last_name, dob = updated_dob)
